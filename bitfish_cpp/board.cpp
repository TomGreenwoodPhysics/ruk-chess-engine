...
void Board::set_fen(const std::string& fen) {
    ...
    for (char c : placement) {
        if (c == '/') {
            ...
            int sq = rank * 8 + file;
            bitboards[piece_from_char(c)] |= bit(sq);
            file += 1;
        }
    }
...
bool Board::is_square_attacked(int sq, int by_colour) const {
    ...
    U64 pawns = bitboards[by_colour == WHITE ? WP : BP];
    while (pawns) {
        auto [psq, next] = pop_lsb(pawns);
        pawns = next;

        if (pawn_attacks_from(psq, by_colour) & bit(sq)) {
            return true;
        }
    }

    U64 knights = bitboards[by_colour == WHITE ? WN : BN];
    while (knights) {
        auto [nsq, next] = pop_lsb(knights);
        knights = next;

        if (knight_attacks(nsq) & bit(sq)) {
            return true;
        }
    }

    U64 bishops = bitboards[by_colour == WHITE ? WB : BB] |
                  bitboards[by_colour == WHITE ? WQ : BQ];

    while (bishops) {
        auto [bsq, next] = pop_lsb(bishops);
        bishops = next;

        if (attacks_from_slider(bsq, BISHOP_DELTAS, occ) & bit(sq)) {
            return true;
        }
    }

    U64 rooks = bitboards[by_colour == WHITE ? WR : BR] |
                bitboards[by_colour == WHITE ? WQ : BQ];

    while (rooks) {
        auto [rsq, next] = pop_lsb(rooks);
        rooks = next;

        if (attacks_from_slider(rsq, ROOK_DELTAS, occ) & bit(sq)) {
            return true;
        }
    }

    int king = king_square(by_colour);
    return king != -1 && (king_attacks(king) & bit(sq));
}
